...
const ShowProduct = () => {
    ...
    const proporty = product != false ? Object.entries(JSON.parse(product[0]?.proporties)).map(([key, value], index) => (
        <ul key={index}>
            <li>{key}: {value}</li>
        </ul>)) : [];
        ...
        <div className={styles.page}>
            ...
            >
                <div className={styles.proporties}>
                    ...
                    <p>price: {product[0].discount === 0 ? product[0].price + '$' : <del>{product[0].price}$</del>}
